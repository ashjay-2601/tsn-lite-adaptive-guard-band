...
module tb_port;
    ...
    wire        tx_tvalid; wire [63:0] tx_tdata;
    ...
    always @(posedge clk) if (rst_n && tx_tvalid) begin
        c = cls_d2;
        if (frame_left[c] == 0 && flen_rd[c] < flen_wr[c]) begin
            frame_left[c] = flen_q[c*64 + flen_rd[c]];
            flen_rd[c] = flen_rd[c] + 1;
        end
        for (i = 0; i < 8; i = i + 1) begin
            if (frame_left[c] > 0 && ref_rd[c] < ref_wr[c]) begin
                got = tx_tdata[8*i +: 8];
